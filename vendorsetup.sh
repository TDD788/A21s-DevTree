...
tar -xJf device/samsung/a21s/source_changes/bootable.tar.xz -C bootable/recovery
cd bootable/recovery
patch -p1 < ../../device/samsung/a21s/colour_fix.diff
cd ../../
...
# MKBOOTIMG
chmod a+x device/samsung/a21s/prebuilt/mkboot/mkbootimg
...
SRC="device/samsung/a21s/source_changes/maintainer/processed_image1.png"
...
export FOX_TARGET_DEVICES="a21s, a21sub, SM-A217M, SM-A217F"
export TARGET_DEVICE_ALT="a21s, a21sub, SM-A217M, SM-A217F"
export FOX_BUILD_DEVICE="a21s, a21sub, SM-A217M, SM-A217F"

# Function to export build variables
export_build_vars() {
    echo -e "${GREEN}Exporting build vars from the a21s tree${RESET}"
...
set_env_var() {
    ...
    echo " export CURR_DEVICE=a21s"
    exit 1
}

# Function to handle mismatched CURR_DEVICE variable
var_not_eq() {
    echo -e "${RED_BACK}CURR_DEVICE not equal to a21s${RESET}"
    ...
    echo -e "${RED}Skipping a21s specific build vars...${RESET}"
}
